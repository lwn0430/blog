...
class ArticleController extends PlatformController
{
    ...
    public function dealEditAction()
    {
        ...
        if($_FILES['thumb']['error']!=4)
        {
            ...
            if($result)
            {
                if($_POST['thumb_back']!='default.jpg')
                {
                    unlink(UPLOADS_DIR.'thumb/'.$_POST['thumb_back']);
                }
                $art['thumb']=$result;      //将新名字记录到数组
            }
            else
            ...
            $art['thumb']=$_POST['thumb_back'];     //以前的缩略图
        }
        $article=Factory::M('ArticleModel');
    ...
    public function realDelAction()
    {
        ...
        $article=Factory::M('ArticleModel');
        //获取要删除文章的thumb名字
        $thumb=$article->getThumb($art_id);
        if($thumb!='default.jpg')
        {
            unlink(UPLOADS_DIR.'thumb/'.$thumb);
        }
